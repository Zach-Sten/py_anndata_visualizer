 ...
 * canvas_webgl.js
 * 
 * Main WebGL canvas code for rendering scatter plots with millions of points.
 * Handles:
 * - WebGL point rendering with GPU acceleration
 * - Embedding transitions (spatial/umap/pca/layout)
 * - Pan, zoom, and rotation navigation
 * - Selection tools (lasso, rectangle, circle, polygon)
 * - Color mapping (categorical obs, continuous gex)
 * - Chunk-based progressive data loading
 * - Sample label overlay rendering
 * - Minimap navigation
 * 
 * Placeholders (replaced via Python .format()):
 *   {iframe_id}       - Unique iframe identifier (JSON-encoded string)
 *   {payload_b64}     - Base64-encoded HTML content for iframe
 *   {debug_log}       - Debug logging statement (empty string if debug=False)
 *   {embeds_js}       - JSON object with embedding metadata
